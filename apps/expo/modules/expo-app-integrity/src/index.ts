...
const iOSAttestKey = async (challenge: string): Promise<string | never> => {
  ...
  try {
    const keyIdentifier = await generateKey()

    await SecureStore.setItemAsync(SECURE_STORAGE_KEYS.INTEGRITY_KEY_IDENTIFIER, keyIdentifier)

    const attestationResult = await IntegrityModule.attestKey(keyIdentifier, challenge)

    return attestationResult
  } catch (error: any) {
    const errorCode = error.message.split(' ')[error.message.split(' ').length - 1]
...
const androidRequestIntegrityVerdict = async (challenge: string, cloudProjectNumber: number): Promise<string | never> => {
  try {
    const verdict = await IntegrityModule.requestIntegrityVerdict(challenge, cloudProjectNumber)
    return verdict
  } catch (error: unknown) {
    // Remove last paren character from stringified error message (i.e., "Error(...)" -> "Error(1234")
    const formattedError = (error as Error).message.substring(0, (error as Error).message.length - 1)
    ...
    if (!AndroidIntegrityErrors[errorCode]) {
      throw unhandledException((error as Error).message)
    }
 ...
 *
 * 1) Generates a key if one doesn't exist and store its identifier in secure store as exposed in peer-dependency `expo-secure-store`. It will use the identifier
 ...
 *      - reduce the possibility of rate-limiting by onboarding users gradually onto App Attest if you have a large user base
...
export async function attestKey(
  /**
   * A cryptographically random value generated on your server,
  ...
  switch (Platform.OS) {
    ...
    case 'android':
      if (!cloudProjectNumber) throw AndroidIntegrityErrors.CLOUD_PROJECT_NUMBER_IS_INVALID
...
export async function generateAssertion(
  ...
  const assertionResult = await IntegrityModule.generateAssertion(keyIdentifier, JSON.stringify(withChallenge))
